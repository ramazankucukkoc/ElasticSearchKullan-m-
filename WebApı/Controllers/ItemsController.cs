...
namespace WebApı.Controllers
{
    ...
    {
        //ElasticClient ve ConnectionSettings kullanımı
        //için NEST kütüphanesini import ederek devam ediyoruz.
        private static readonly ConnectionSettings connectionSettings =
            new ConnectionSettings(new Uri("http://localhost:9200/"))
            .DefaultIndex("esearchitems")
            .DefaultMappingFor<Item>(m => m.PropertyName(p => p.Id, "id"));
            ...
            if (!elasticClient.Indices.Exists("esearchitems").Exists)
            {
                elasticClient.Indices.Create("esearchitems",
                    index => index.Map<Item>(
                        x => x.AutoMap()));
                elasticClient.Bulk(b => b.Index("esearchitems").IndexMany(ItemService.GetItems()));
